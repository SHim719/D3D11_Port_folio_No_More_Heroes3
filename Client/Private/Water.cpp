...
HRESULT CWater::Initialize(void* pArg)
{
	if (pArg != nullptr)
	{
		m_tWaterDesc.fLightColor = ((WATER_DESCS*)pArg)->fLightColor;
		m_tWaterDesc.fVoronoiScale = ((WATER_DESCS*)pArg)->fVoronoiScale;
		m_tWaterDesc.fWaterColor = ((WATER_DESCS*)pArg)->fWaterColor;
		m_tWaterDesc.fWaterNoiseScale = ((WATER_DESCS*)pArg)->fWaterNoiseScale;
		m_tWaterDesc.vFlowSpeed = ((WATER_DESCS*)pArg)->vFlowSpeed;
	}
...
{
	if (FAILED(m_pShader->Bind_RawValue("g_fFlowSpeed", &m_tWaterDesc.vFlowSpeed, sizeof(_float2))))
		return E_FAIL;

	if (FAILED(m_pShader->Bind_RawValue("g_fVoronoiScale", &m_tWaterDesc.fVoronoiScale, sizeof(_float))))
		return E_FAIL;

	if (FAILED(m_pShader->Bind_RawValue("g_fWaterNoiseScale", &m_tWaterDesc.fWaterNoiseScale, sizeof(_float))))
		return E_FAIL;

	if (FAILED(m_pShader->Bind_RawValue("g_vWaterColor", &m_tWaterDesc.fWaterColor, sizeof(_float4))))
		return E_FAIL;

	if (FAILED(m_pShader->Bind_RawValue("g_vLightColor", &m_tWaterDesc.fLightColor, sizeof(_float4))))
		return E_FAIL;
