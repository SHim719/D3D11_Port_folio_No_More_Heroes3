...
void CTexture::Get_Textures(vector<ID3D11ShaderResourceView*>* pVec)
{
	pVec->reserve(m_iNumTextures);

	for (auto& Texture : m_SRVs)
	{
		pVec->emplace_back(Texture);
		Safe_AddRef(Texture);
	}
}
...
CTexture* CTexture::Create_ExtraMaterial(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _tchar* pTextureFilePath)
{
	CTexture* pInstance = new CTexture(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(pTextureFilePath, 1)))
		Safe_Release(pInstance);
